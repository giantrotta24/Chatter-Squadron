...
var StateNode = /** @class */ (function () {
    ...
    StateNode.prototype.getDelayedTransitions = function () {
        ...
        if (utils_1.isArray(afterConfig)) {
            return afterConfig.map(function (delayedTransition, i) {
                ...
                return __assign({ event: event }, delayedTransition, { target: target === undefined ? undefined : utils_1.toArray(target), cond: utils_1.toGuard(delayedTransition.cond, guards), actions: utils_1.toArray(delayedTransition.actions).map(function (action) {
                        return actions_1.toActionObject(action);
                    }) });
            });
        }
        var allDelayedTransitions = utils_1.flatten(utils_1.keys(afterConfig).map(function (delayKey) {
            ...
            return delayedTransitions.map(function (transition) { return (__assign({ event: event,
                delay: delay }, transition, { target: transition.target === undefined
                    ? transition.target
    ...
    StateNode.prototype.transitionLeafNode = function (stateValue, state, eventObject) {
        ...
        if (!next.tree) {
            var _a = this.next(state, eventObject), actions = _a.actions, tree = _a.tree;
            return {
                tree: tree,
                source: state,
                actions: actions
    ...
    StateNode.prototype.transitionParallelNode = function (stateValue, state, eventObject) {
        ...
        if (!willTransition) {
            var _d = this.next(state, eventObject), actions = _d.actions, tree = _d.tree;
            return {
                tree: tree,
                source: state,
                actions: actions
        ...
        var allResolvedTrees = utils_1.keys(transitionMap).map(function (key) {
            ...
            var subValue = utils_1.path(_this.path)(state.value)[key];
    ...
    StateNode.prototype.formatTransition = function (target, transitionConfig, event) {
        ...
        return __assign({}, transitionConfig, { actions: actions_1.toActionObjects(utils_1.toArray(transitionConfig.actions)), cond: utils_1.toGuard(transitionConfig.cond, guards), target: isTargetless ? undefined : formattedTargets, internal: (isTargetless && internal === undefined) || internal, event: event });
    };
